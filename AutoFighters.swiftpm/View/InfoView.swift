        ...
        ZStack{
            ...
            VStack{
                ...
                TabView{
                        ...
                        ParagraphImageView(title: "How to create your automaton", imgNames: ["create","line","erase","Run"], texts: ["The first step is to create the states, you'll notice that the first one will be colored green this is your initial state, meaning that the execution will start there. If you delete the initial state, the next state created will become the initial one.","In order to create the conditions just select the type and click in sequence the two states you want to link, starting by the origin. But, you can create only one conditon between the sames states.","To remove a state just click on the icon, and select the state to be removed.","To start the battle just click on RUN."])
                        ParagraphImageView(title: "In this automaton there are 5 states:", imgNames: ["up","down","left","rigth","impulse"], texts: ["Up: the roomba moves upward.","Down: the roomba moves downward.","Left: the roomba moves leftward.","Right: the roomba moves rightward.","Impulse: the roomba launches itself in the direction he's facing."])
                        ParagraphImageView(title: "In this automaton there are 3 conditions", imgNames: ["time","distance","touch"], texts: ["Time: sets a timer for the roomba's current action. ","Distance: sets a distance for the roomba's current action","Touch: the roomba will execute the current action until it touches another roomba."])
                }
                .tabViewStyle(.page)
            }
        }
